...
const AIChat: React.FC<AIChatProps> = ({ user }) => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      // Inicialização robusta usando a chave injetada pelo Vite
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          HABILIDADES:
          - Você tem acesso ao Google Maps e Google Search.
          - Use o Google Maps para validar endereços de unidades quando solicitado.
          - Se o usuário perguntar sobre unidades, sugira localizá-las no mapa.
      ...
      // Extrair links de grounding (Google Maps / Search)
      ...
      if (!process.env.API_KEY) errorMsg = "A chave de API da IA não foi configurada corretamente no servidor.";
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[500] flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 w-[350px] sm:w-[400px] h-[550px] bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
          <div className="p-6 bg-slate-900 text-white flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-black text-sm tracking-tight">VeroPonto Intelligence</h3>
                    ...
                    {msg.links && msg.links.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-slate-100 space-y-2">
                        <p className="text-[9px] font-black uppercase tracking-widest opacity-50 flex items-center gap-1">
                          <MapPin size={10} /> Referências do Mapa:
                        </p>
                        {msg.links.map((link, idx) => (
                          <a 
                            ...
                            className="flex items-center justify-between gap-2 p-2 bg-slate-50 rounded-lg border border-slate-100 hover:bg-indigo-50 hover:border-indigo-100 transition-all text-indigo-600 font-bold text-[10px]"
                          >
          ...
          <div className="p-4 bg-white border-t border-slate-100">
            <form onSubmit={handleSendMessage} className="relative">
              <input 
                ...
                placeholder="Pergunte sobre uma unidade ou endereço..."
