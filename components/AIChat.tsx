...
const AIChat: React.FC<AIChatProps> = ({ user, isPro }) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: `Olá ${user.name.split(' ')[0]}! Sou o GeoPoint AI. Como posso ajudar com sua jornada hoje?` }
  ]);
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const result = await ai.models.generateContentStream({
        ...
        config: {
          systemInstruction: `Você é o "GeoPoint AI", assistente do sistema de ponto GeoPoint. 
          Usuário: ${user.name} (${user.role}).
          Seja direto, profissional e ajude com dúvidas de ponto e CLT.`,
          temperature: 0.7,
        },
      });
      ...
      if (error.message?.includes('API_KEY_INVALID')) {
        errorMsg = "A chave de API configurada é inválida. Verifique suas configurações.";
      }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[5000] flex flex-col items-end">
        ...
        <div className="mb-4 w-[calc(100vw-2rem)] sm:w-[380px] h-[calc(100dvh-12rem)] sm:h-[520px] bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-2xl rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-white/20 dark:border-white/5 flex flex-col overflow-hidden animate-in slide-in-from-bottom-6 duration-500">
          ...
          <div className="p-6 flex justify-between items-center shrink-0 bg-gradient-to-r from-[#050505] to-[#1a1a1a] text-white">
            <div className="flex items-center gap-3">
              <div className="bg-gradient-to-br from-teal-400 to-indigo-500 p-2 rounded-2xl">
                <Sparkles size={18} className="text-white animate-pulse" />
              </div>
              <div>
                <h3 className="font-black text-xs uppercase tracking-widest">GeoPoint AI</h3>
                <div className="flex items-center gap-1">
                   <span className={`w-1.5 h-1.5 rounded-full ${apiKeyError ? 'bg-red-500' : 'bg-emerald-500 animate-pulse'}`}></span>
                   <p className="text-[8px] font-bold text-slate-400 uppercase tracking-tighter">
                     {apiKeyError ? 'API KEY AUSENTE' : 'Gemini 3 Ativo'}
                   </p>
                </div>
              </div>
            </div>
            ...
            {apiKeyError && (
              <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-2xl flex gap-3 text-red-500">
                ...
                <div>
                  ...
                  <p className="text-[9px] font-medium leading-relaxed mt-1">A chave da Gemini não foi encontrada no Netlify. Adicione a variável API_KEY para ativar a IA.</p>
                </div>
              </div>
            )}
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-8 h-8 rounded-xl shrink-0 flex items-center justify-center shadow-sm ${
                  msg.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-teal-500 text-white'
                }`}>
          ...
          <div className="p-5 bg-slate-50/50 dark:bg-white/5 border-t border-slate-100 dark:border-white/5">
            <form onSubmit={handleSendMessage} className="relative group">
              <input 
                ...
                placeholder={apiKeyError ? "IA Desativada..." : "Pergunte sobre seu ponto..."} 
                className="w-full pl-5 pr-14 py-4 bg-white dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-[1.8rem] text-xs font-bold outline-none focus:ring-2 focus:ring-teal-500/50 transition-all dark:text-white disabled:opacity-50"
              />
              <button 
                disabled={isTyping || !input.trim() || apiKeyError} 
                className="absolute right-2 top-1/2 -translate-y-1/2 p-2.5 bg-gradient-to-br from-teal-400 to-indigo-600 text-white rounded-2xl shadow-lg disabled:opacity-30 transition-all hover:scale-105"
              >
                <Send size={16} />
              </button>
            </form>
      ...
      <button onClick={() => setIsOpen(!isOpen)} className={`w-16 h-16 rounded-[1.8rem] shadow-xl flex items-center justify-center transition-all duration-500 hover:scale-110 border-4 border-white dark:border-[#050505] ${isOpen ? 'bg-slate-900 text-white' : 'bg-gradient-to-br from-teal-400 to-indigo-600 text-white'}`}>
