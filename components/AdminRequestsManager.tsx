...
const AdminRequestsManager: React.FC<AdminRequestsManagerProps> = ({ workspaceId }) => {
  ...
  useEffect(() => {
    ...
    // Sincronização de foco essencial para APK/WebView
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      {loading && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[5000] bg-indigo-600 text-white px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center gap-2">
          <RefreshCw size={12} className="animate-spin" /> Sincronizando Central...
        </div>
      )}
      ...
      <div className="relative group max-w-lg">
        ...
        <input 
          ...
          className="w-full pl-14 pr-6 py-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 rounded-[1.5rem] text-xs font-bold outline-none dark:text-white shadow-sm"
        />
      </div>
          ...
          >
            ...
            <div className="flex justify-between items-start mb-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-white/5 text-slate-400 flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition-all">
                  <UserIcon size={24} />
                </div>
                <div>
            ...
            <div className="bg-slate-50/50 dark:bg-white/5 p-4 rounded-2xl space-y-3 flex-1 mb-6">
              <div className="flex items-center gap-2 text-slate-400">
                <Calendar size={12} />
                <span className="text-[10px] font-black uppercase tracking-widest">Data: {new Date(req.date).toLocaleDateString()}</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2 italic leading-relaxed">
                "{req.description || 'Sem descrição'}"
              </p>
            </div>
            <button className="w-full py-4 bg-slate-900 text-white dark:bg-white dark:text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 transition-all shadow-lg">
      ...
      {viewingRequest && (
        <div className="fixed inset-0 z-[3000] bg-black/80 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden border border-white/5 flex flex-col max-h-[90vh]">
            <div className="p-8 border-b border-slate-50 dark:border-white/5 flex justify-between items-center bg-slate-50/50 dark:bg-white/5">
              <div className="flex items-center gap-4">
                ...
                <h3 className="font-black text-sm uppercase tracking-widest text-slate-900 dark:text-white">Análise</h3>
              </div>
              <button onClick={() => setViewingRequest(null)} className="p-3 text-slate-400 hover:text-red-500 transition-colors"><X size={24}/></button>
            </div>
            <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div>
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1">{viewingRequest.type}</p>
                </div>
              </div>
                ...
                <div className="aspect-video bg-slate-950 rounded-[2.5rem] overflow-hidden border-4 border-slate-50 dark:border-white/5 group relative shadow-inner">
                  ...
                  {viewingRequest.attachment && (
                    <a href={viewingRequest.attachment} target="_blank" className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white gap-2 font-black text-[10px] uppercase tracking-widest">
                      <ExternalLink size={20} /> Ver em Nova Aba
                    </a>
                  )}
                </div>
              </div>
              <div className="bg-slate-50 dark:bg-white/5 p-8 rounded-[2.5rem] border border-slate-100 dark:border-white/10 italic text-slate-700 dark:text-slate-300">
                "{viewingRequest.description || 'Nenhuma descrição fornecida.'}"
              </div>
            </div>
              ...
              {viewingRequest.status === 'pending' ? (
                <>
                  <button 
                    ...
                    className="flex-1 py-5 bg-white dark:bg-red-500/10 text-red-600 rounded-2xl font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 border border-red-100"
                  >
                    {actionLoading === viewingRequest.id ? <Loader2 size={18} className="animate-spin" /> : <XCircle size={18} />} Indeferir
                  </button>
                  <button 
                    disabled={actionLoading !== null}
                    onClick={() => handleUpdateStatus(viewingRequest.id, 'approved')}
                    className="flex-[1.5] py-5 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-xl"
                  >
                    {actionLoading === viewingRequest.id ? <Loader2 size={18} className="animate-spin" /> : <CheckCircle2 size={18} />} Deferir
                  </button>
                </>
              ) : (
                <div className={`w-full py-6 rounded-2xl font-black text-xs uppercase tracking-[0.3em] flex items-center justify-center gap-3 border ${
                  ...
                  Solicitação {viewingRequest.status === 'approved' ? 'Deferida' : 'Indeferida'}
                </div>
              )}
            </div>
          </div>
