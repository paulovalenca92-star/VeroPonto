...
const MonthlyReport: React.FC<MonthlyReportProps> = ({ user, records, onClose }) => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      doc.text(`Mês de Referência: ${month}/${year}`, 150, 25);
      ...
      doc.text(`Protocolo de Autenticidade: ${protocol}`, 20, doc.internal.pageSize.height - 15);
      doc.text(`Gerado em: ${new Date().toLocaleString()}`, 150, doc.internal.pageSize.height - 15);
  ...
  return (
    <div className="fixed inset-0 z-[6000] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in">
      <div className="bg-[#111111] border border-white/10 w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-gradient-to-br from-[#2DD4BF] to-[#4F46E5] rounded-xl text-white">
              <FileText size={20} />
            </div>
            <h3 className="font-black text-[10px] uppercase tracking-[0.2em] text-white">Relatório Mensal</h3>
          </div>
        ...
        <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar flex-1">
          <div className="space-y-4">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 flex items-center gap-2">
              <Calendar size={14} /> Selecione o Mês
            </label>
            <input 
              type="month" 
              value={selectedMonth}
              onChange={(e) => setSelectedMonth(e.target.value)}
              className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl font-black text-white outline-none focus:border-[#2DD4BF]/50"
            />
          </div>

          <div className="space-y-4">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Assinatura Digital</label>
            <div className="bg-white rounded-3xl overflow-hidden border-2 border-white/10 h-48 relative shadow-inner">
              <SignatureCanvas 
                ref={sigPad}
                penColor="black"
            ...
            <p className="text-[9px] text-slate-500 text-center font-bold uppercase tracking-widest italic">Assine com o dedo ou mouse dentro do quadro branco</p>
          </div>
        </div>
        ...
        <div className="p-8 bg-white/5 border-t border-white/5">
          ...
          >
            ...
            {isGenerating ? "GERANDO DOCUMENTO..." : "GERAR PDF ASSINADO"}
          </button>
        </div>
      </div>
    </div>
