    ...
    <div className={`min-h-[100dvh] transition-colors ${theme === 'dark' ? 'bg-[#0a0f1a] text-slate-100' : 'bg-[#f8fafc] text-slate-900'}`}>
      ...
      {swUpdate && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[9999] w-[calc(100%-2rem)] max-w-sm">
          <div className="bg-teal-600 text-white p-4 rounded-2xl shadow-2xl flex items-center justify-between border border-white/20">
             ...
             <button onClick={() => window.location.reload()} className="px-4 py-2 bg-white text-teal-600 rounded-xl font-black text-[10px] uppercase">Reiniciar</button>
          </div>
        </div>
      )}
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[1000] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-[380px] rounded-[2.5rem] shadow-2xl p-8 border border-white/5">
            <div className="flex justify-between items-center mb-8">
               <h3 className="font-black text-xs uppercase tracking-widest">Preferências</h3>
               <button onClick={() => setShowSettings(false)} className="p-2 text-slate-400"><X size={20} /></button>
            </div>
            <div className="p-1.5 rounded-2xl flex gap-1 bg-slate-100 dark:bg-black/40">
              <button onClick={() => setTheme('light')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 ${theme === 'light' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}><Sun size={12} /> Claro</button>
              <button onClick={() => setTheme('dark')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 ${theme === 'dark' ? 'bg-[#111] text-white shadow-sm' : 'text-slate-500'}`}><Moon size={12} /> Escuro</button>
            </div>
            <button onClick={() => setShowSettings(false)} className="w-full mt-6 py-4 bg-slate-900 text-white dark:bg-white dark:text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-widest">Concluir</button>
          </div>
        </div>
      )}
